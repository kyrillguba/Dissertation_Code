"""Train U-TGV with the shared denoising experiment pipeline."""
...
def train_u_tgv(args: argparse.Namespace, train_loader=None, val_loader=None):
    """Train U-TGV and return ``(model, history_dataframe)``."""
    apply_common_defaults(args, checkpoint_dir="checkpoints/u_tgv")
    for name, value in {
        "num_pdhg_iters": 64,
        "map_scale": 0.1,
        "pdhg_tau": None,
    ...
    # Delegate all controlled experiment mechanics to the shared pipeline.
    return train_denoising_model(
        args,
        model_name="u_tgv",
        train_loader=train_loader,
        val_loader=val_loader,
        model_factory=lambda: UTGVModel(
...
def parse_args() -> argparse.Namespace:
    """Parse command-line options for a U-TGV training run."""
    parser = argparse.ArgumentParser(description="Train U-TGV on SeaTurtleID2022.")
    add_common_training_arguments(parser, checkpoint_dir="checkpoints/u_tgv")
