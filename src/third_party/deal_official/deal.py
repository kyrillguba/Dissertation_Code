"""Vendored implementation of the official Deep Equilibrium Algorithmic Learning model.
"""
import torch
...
class DEAL(nn.Module):
    """Official DEAL regulariser and nested inverse-problem solver."""
    def __init__(self, color) -> None:
        """Construct grayscale or colour DEAL operators, splines, and diagnostics."""
        super().__init__() 
    ...
    def cal_lambda(self, sigma):
        """Evaluate the learned noise-to-regularisation spline."""
        self.lmbda = self.spline_lambda(sigma) 
    
    def cal_scaling(self, sigma):
        """Evaluate per-channel activation scaling for the supplied noise level."""
    ...
    def cg(self, b, x0, max_iter, eps=1e-5, H=lambda x: x, Ht=lambda x: x, dims=(1, 2, 3)):
        ...
        """Solve the inverse-problem normal equations with batched active-set CG."""
        b = b / (1 + self.lmbda)
        x = x0.clone() 
        correct_idx_mask = [i for i in range(x.size(0))]
    ...
    def denoise(self, y, sigma):
        ...
        """Denoise an image batch using the released DEAL fixed-point iteration."""
        ...
        def backward_hook1(grad):
            """Inspect gradient values during the optional debugging hook."""
            self.cal_mask(d_k)      
    ...
    def solve_inverse_problem(self, y, H, Ht, sigma, lmbda, eps_in=1e-8, eps_out=1e-5, path=False, x_init=None, verbose=False):
        ...
        """Solve a general linear inverse problem with nested DEAL iterations."""
        self.W1.spectral_norm()
