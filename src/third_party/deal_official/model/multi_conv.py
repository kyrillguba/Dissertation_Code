"""Normalized multi-layer convolution operators used by official DEAL."""
import torch
...
class MultiConv2d(nn.Module):
    """Chain convolutions and expose its explicitly implemented transpose."""
    ...
    def forward(self, x):
        """Apply the normalized convolutional chain."""
    ...
    def convolution(self, x):
        ...
        """Apply all normalized convolution layers in forward order."""
    ...
    def transpose(self, x):
        ...
        # nb the spectral norm of the convolution has to be upated before
        """Apply transposed convolutions in reverse order."""
        x = x / torch.sqrt(self.L)
    ...
    def spectrum(self):
        """Return the Fourier spectrum of the equivalent normal-operator kernel."""
        kernel = self.get_kernel_WtW()
        padding = (self.sn_size - 1) // 2 - self.padding_total
    ...
    def get_filters(self):
        ...
        """Recover equivalent analysis filters from a Dirac impulse response."""
    ...
    def get_kernel_WtW(self):
        """Return the Dirac response of the convolutional normal operator."""
...
class ZeroMean(nn.Module):
    """Parametrization that removes each filter's spatial-channel mean."""
    def forward(self, X):
        """Subtract the per-filter mean from a weight tensor."""
