"""Custom autograd kernels for the official DEAL spline activations."""
import torch
...
class LinearSpline_Func(torch.autograd.Function):
    ...
    def forward(ctx, x, coefficients, x_min, x_max, num_knots, zero_knot_indexes):
        ...
        """Evaluate piecewise-linear splines and cache interpolation data."""
    ...
    def backward(ctx, grad_out):
        ...
        """Accumulate gradients for linear-spline inputs and active coefficients."""
...
class LinearSplineDerivative_Func(torch.autograd.Function):
    ...
    def forward(ctx, x, coefficients, x_min, x_max, num_knots, zero_knot_indexes):
        ...
        # The value of the spline at any x is a combination 
        # of at most two coefficients
        """Evaluate the piecewise-constant derivative of a linear spline."""
        step_size = (x_max - x_min) / (num_knots - 1)
        x_clamped = x.clamp(min=x_min.item(), max=x_max.item() - step_size.item())
    ...
    def backward(ctx, grad_out):
        ...
        """Accumulate coefficient gradients for the spline derivative."""
        fracs, coefficients, indexes, step_size = ctx.saved_tensors
        grad_x = 0 * grad_out
...
class Quadratic_Spline_Func(torch.autograd.Function):
    ...
    def forward(ctx, x, coefficients, x_min, x_max, num_knots, zero_knot_indexes):
        ...
        """Evaluate quadratic B-splines and cache their local basis weights."""
        step_size = (x_max - x_min) / (num_knots - 1)
    ...
    def backward(ctx, grad_out):
        ...
        """Accumulate input and coefficient gradients for quadratic splines."""
