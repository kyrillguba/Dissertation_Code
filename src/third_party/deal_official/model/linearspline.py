"""Learnable linear-spline activations used by the official DEAL implementation."""
import torch
import torch.nn as nn
import torch.nn.functional as F
from torch import Tensor
from abc import ABC, abstractproperty, abstractmethod
from . import spline_autograd_func
...
class LinearSpline(ABC, nn.Module):
    ...
    def __init__(self, num_activations, num_knots, x_min, x_max, init, slope_max=None, slope_min=None, clamp=True, **kwargs):
        ...
        """Initialise learnable spline knots, constraints, and activation ranges."""
        super().__init__()
    ...
    @property
    def device(self):
        """Return the device holding the learnable spline coefficients."""
        return self.coefficients.device
    
    def hyper_param_to_device(self):
        """Move cached spline constants to the coefficient device."""
        device = self.device
